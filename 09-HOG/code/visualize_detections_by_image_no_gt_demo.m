...
function visualize_detections_by_image_no_gt_demo(bboxes, confidences, image_ids, test_scn_path,random_number)
...
test_files = dir(fullfile(test_scn_path, '*.png'));
num_test_images = length(test_files);

%for i=1:num_test_images
   cur_test_image = imread( fullfile( test_scn_path, test_files(random_number).name));
      
   cur_detections = strcmp(test_files(random_number).name, image_ids);
   ...
   axis image;
   axis off;
   title(sprintf('image: "%s" green=detection', test_files(random_number).name),'interpreter','none');
   ...
   imwrite(detection_image, sprintf('visualizations/detections_%s.png', test_files(random_number).name))
